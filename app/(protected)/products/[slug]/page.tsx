...
export default async function ProductPage({
  ...
  const {fetchProductBySlug, product} = useProductQuery()
                ...
                <div className='grid gap-8 md:grid-cols-2'>
                    ...
                    <div className='md:py-8'>
                        ...
                        <div className='flex gap-2.5'>
                            <AddToCart currency='USD' description={product?.description} image={product?.images[0]} name={product?.name} price={product?.discountPrice} key={product?._id} price_id={product?._id} />
                            <Button variant='secondary' className='hover:bg-gray-300 font-bold rounded-lg' >Checkout Now</Button>
                        </div>
                        <p className='mt-12 text-base text-gray-500 leading-relaxed tracking-wide'>{product?.description}</p>
                    </div>
                </div>
            </div>
        </div>
